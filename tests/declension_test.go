...
// ————————————————————————————————————————————————————————————————
// Тест: базовые склонения ФИО через petrovich
// ————————————————————————————————————————————————————————————————
func TestDeclension_BasicCases(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		caseName string
	...
	}{
		{
			...
			format:   "ф и о",
			expected: "Иванова Ивана Ивановича",
		},
		{
			name:     "Дательный падеж, краткий формат",
			input:    "Иванов Иван Иванович",
			caseName: "дательный",
			format:   "ф и.о.",
			expected: "Иванову И.И.",
		},
		{
			name:     "Винительный падеж, только фамилия",
			input:    "Иванов Иван Иванович",
		...
		{
			name:     "Творительный падеж, инициалы перед фамилией",
			input:    "Иванов Иван Иванович",
			caseName: "творительный",
			format:   "и.о. ф",
			expected: "И.И. Ивановым",
		},
		{
			name:     "Предложный падеж, стандартный формат по умолчанию",
