...
// intToWordsRu — простая локальная реализация чисел словами (до 9999).
// Для производственных нужд можно заменить на ntw.IntegerToRuRu.
func intToWordsRu(n int, gender, caseName string) string {
	if n == 0 {
		return "ноль"
	}

	onesMasc := []string{"", "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять"}
